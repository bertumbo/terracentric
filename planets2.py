...
class LED:
    ...
    def init_blender(self):
        ...
        bpy.ops.mesh.primitive_cube_add(size=2, enter_editmode=False, align='WORLD', location=(x, y, 0),
                                        rotation=(0, 0, self.phi), scale=(0.2, 0.2, 0.2))

        self.ob = bpy.context.active_object
        self.ob.name = self.IDD
        self.ob_name = self.IDD
    ...
    def refresh_blender(self):
        ...
        diffuse.inputs['Color'].default_value = (self.ir / 255, self.ig / 255, self.ib / 255, 1)  # last value alpha
...
def draw_fig(timestamp=time.time()):
    ...
    for led in LED.instances:
        ...
        draw.regular_polygon(bounding_circle=((x * sc + offx), (y * sc + offy), 8), n_sides=4,
                             rotation=-np.rad2deg(led.phi), fill=(int(led.ir), int(led.ig), int(led.ib)), outline=None)
...
for i in range(10):
    ...
    scene.render.filepath = 'blender%s.png' % i

    bpy.ops.render.render(write_still=1)
    # blImList.append(blIm)

    # with Image.open("C:\\blender%s.png" % i) as im:
    # blImList.append(im)

    # except:
    print("skipping blender stuff")

###finish
name = "test11"
filename = "%s.gif" % name
out = imList[0].save(filename, save_all=True, append_images=imList[1:], optimize=True, duration=30, loop=0)
# out2 = blImList[0].save("blenderout.gif", save_all=True, append_images=blImList[1:], optimize=True, duration=30, loop=0)
